...
class GLTFLoader
{
  public:
	 ...
	 */
	std::unique_ptr<sg::SubMesh> read_model_from_file(const std::string &file_name, uint32_t index,  bool add_flat_vertices=false, bool mesh_shader_buffer=false);
  ...
  private:
	...
	std::unique_ptr<sg::SubMesh> load_model(uint32_t index, bool add_flat_vertices, bool mesh_shader_buffer=false);
};
}        // namespace vkb
